  ...
  return (
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <button
          onClick={openModal}
          className="inline-flex h-10 items-center gap-2 rounded-full bg-[#2F5D3A] px-5 text-sm font-medium text-white transition-colors hover:bg-[#24492D]"
        >
...
function SaveBtn({ onClick, pending, label, c }: { onClick: () => void; pending: boolean; label?: string; c: Conn }) {
  return (
    <button onClick={onClick} disabled={pending} className="flex-1 rounded-xl bg-[#2F5D3A] py-2.5 text-sm font-medium text-white hover:bg-[#24492D] disabled:opacity-50">
...
function EmptyState({ onAdd, c }: { onAdd: () => void; c: Conn }) {
  return (
    <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)] p-10 text-center">
      ...
      <button onClick={onAdd} className="mt-5 inline-flex h-10 items-center justify-center rounded-full bg-[#2F5D3A] px-6 text-sm font-medium text-white transition-colors hover:bg-[#24492D]">
        {c.addDevice}
      </button>
    </div>
  )
