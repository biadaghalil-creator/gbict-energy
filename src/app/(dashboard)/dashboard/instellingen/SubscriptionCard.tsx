...
export default function SubscriptionCard({ status, plan, currentPeriodEnd, trialEndsAt }: Props) {
  ...
  return (
    ...
    >
      ...
      <h2 className="relative text-[15px] font-bold tracking-tight text-[var(--text)]">Abonnement</h2>
            ...
            {PLAN_OPTIONS.map((p) => (
              <div key={p.id} className="flex flex-col rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-5">
                <div className="flex items-baseline justify-between">
                  <span className="text-[15px] font-bold text-[var(--text)]">{p.name}</span>
                  <span className="text-[15px] font-bold text-[var(--text)]">€{p.price}<span className="text-[12px] font-medium text-[var(--text-faint)]">/mnd</span></span>
                </div>
