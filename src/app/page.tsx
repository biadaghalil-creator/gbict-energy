...
const btnGhost =
  ...
  "text-white text-[15px] font-medium hover:bg-white/[0.06] transition-colors";
...
function SectionHead({ eyebrow, title, sub }: { eyebrow?: string; title: string; sub?: string }) {
  return (
    <div className="text-center">
      ...
      {sub && <p className="mx-auto mt-5 max-w-[560px] text-[17px] leading-[1.6] text-slate-400">{sub}</p>}
    </div>
  );
}
    ...
    <div className="relative">
      ...
      {showPicker && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-36 overflow-hidden rounded-xl border border-white/[0.08] bg-[#0D0E16] shadow-2xl">
            {locales.map(l => (
              <button
                key={l}
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0D0E16] p-8 shadow-[0_24px_60px_-32px_rgba(0,0,0,0.9)]">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <span className="flex items-center gap-2 text-[12px] font-medium tracking-[0.02em] text-slate-400">
      ...
      <div className="mb-7 flex gap-3">
        <div className="flex-1 rounded-xl border border-white/[0.06] bg-white/[0.025] px-4 py-3.5">
          <div className="text-[11px] text-slate-500">Spot price now</div>
          <div className="mt-1.5 font-mono text-[18px]">€0.0421<span className="text-[11px] text-slate-500">/kWh</span></div>
        </div>
        <div className="flex-1 rounded-xl border border-white/[0.06] bg-white/[0.025] px-4 py-3.5">
          <div className="text-[11px] text-slate-500">Saved today</div>
          <div className="mt-1.5 font-mono text-[18px] text-emerald-400">€2.14</div>
        </div>
      </div>
      <div className="mb-3 text-[10.5px] font-bold uppercase tracking-[0.12em] text-slate-400">Optimization schedule · today</div>
    ...
    <section id="top" className="flex min-h-[calc(100vh-68px)] items-center py-40">
      <div className="mx-auto grid w-full max-w-[1140px] grid-cols-1 items-center gap-[72px] px-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <p className="mt-7 max-w-[520px] text-[18px] font-medium leading-[1.55] text-slate-400">
          ...
          <p className="mt-6 text-[13.5px] text-slate-500">14 days free · no credit card · cancel anytime</p>
        </div>
        <Dashboard />
      </div>
    </section>
          ...
          {PLANS.map((p) => (
            <div key={p.name} className={"relative flex flex-col rounded-2xl border bg-[#0D0E16]/70 p-10 backdrop-blur " + (p.popular ? "border-violet-500/35 bg-[#0D0E16]/85 shadow-[inset_0_1px_0_rgba(139,92,246,0.5)]" : "border-white/[0.06]")}>
              ...
              <div className="mb-0.5 mt-5 text-[44px] font-extrabold leading-none tracking-[-0.03em]">
                {p.price}{p.period && <span className="text-[14px] font-medium text-slate-500">{p.period}</span>}
              </div>
              <ul className="my-7 flex-1 list-none p-0">
                {p.features.map((f) => (
                  <li key={f} className="flex items-center gap-2.5 py-2 text-[14px] text-slate-300"><Check className="h-[15px] w-[15px] shrink-0 text-emerald-400" />{f}</li>
                ))}
              </ul>
              <a href="/signup" className={(p.popular ? btnPrimary : btnGhost) + " w-full"}>{p.cta}</a>
              <div className="mt-3.5 text-center text-[12px] text-slate-500">{p.note}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
          {stats.map((s) => (
            <div key={s.l}>
              ...
              <div className="mt-3 max-w-[240px] text-[14px] leading-[1.5] text-slate-400">{s.l}</div>
            </div>
          ))}
        </div>
...
function Footer() {
  const cols: [string, [string, string][]][] = [
    ["Product",          [["Dashboard", "/dashboard"], ["Pricing", "#pricing"], ["API Docs", "/docs"], ["Changelog", "/changelog"]]],
  ...
  return (
    <footer id="footer" className="relative z-[2] border-t border-white/[0.06] pb-10 pt-16">
      <div className="mx-auto max-w-[1140px] px-6">
        <div className="grid grid-cols-1 gap-9 md:grid-cols-[1.6fr_1fr_1fr_1.2fr]">
          <div>
            ...
            <p className="mb-[18px] max-w-[280px] text-[14px] leading-[1.55] text-slate-400">The operating system for your home energy.</p>
            <div className="flex gap-2.5">
              {[Share2, Link, GitFork].map((Ic, i) => (
                <a key={i} href="#" className="grid h-9 w-9 place-items-center rounded-xl border border-white/[0.06] bg-[#0D0E16]/70 text-slate-400 transition-colors hover:border-white/10 hover:text-slate-100"><Ic className="h-[17px] w-[17px]" /></a>
              ))}
            </div>
          </div>
          {cols.map(([h, links]) => (
            <div key={h}>
              <h4 className="mb-4 text-[12px] font-bold uppercase tracking-[0.12em] text-slate-500">{h}</h4>
              {links.map(([label, href]) => (
                <a key={label} href={href} className="block py-1.5 text-[14px] text-slate-500 transition-colors hover:text-slate-100">{label}</a>
              ))}
            </div>
          ))}
        </div>
        <div className="mt-12 flex flex-col items-center justify-between gap-3.5 border-t border-white/[0.06] pt-6 text-[13px] text-slate-600 md:flex-row">
