...
function AuthScreen({ onAuthed }) {
  ...
  const submit = async (e) => {
    ...
    if (!email || !password) {
      setErr("Vul je e-mail en wachtwoord in.");
      return;
    }
    setErr("");
    setBusy(true);
    try {
      ...
      if (isSignup && data.needsConfirm) {
        setErr("Check je e-mail om je account te bevestigen, log daarna in.");
        setBusy(false);
        return;
      }
      try {
