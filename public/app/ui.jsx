// ui.jsx — shared primitives: icons, device frame, status bar, building blocks.
// Exports to window for the other babel scripts.
const { useState, useEffect, useRef } = React;

/* ───────────────────────── Icons (stroke, lucide-like) ───────────────────── */
const ICONS = {
...
function useCountUp(target, { run = true, dur = 1100, decimals = 0 } = {}) {
  ...
  useEffect(() => {
    ...
    // safety: timers fire even when rAF is throttled offscreen, so the
    // number never sticks at 0 if the animation clock never advances.
...
function StatusBar() {
  return (
    <div className="statusbar">
      <div className="t num">9:41</div>
      <div className="r">
        <svg width="18" height="12" viewBox="0 0 18 12" fill="currentColor"><rect x="0" y="7" width="3" height="5" rx=".6"/><rect x="4.6" y="4.6" width="3" height="7.4" rx=".6"/><rect x="9.2" y="2.3" width="3" height="9.7" rx=".6"/><rect x="13.8" y="0" width="3" height="12" rx=".6"/></svg>
        <svg width="16" height="12" viewBox="0 0 16 12" fill="currentColor"><path d="M8 2.6c2.1 0 4 .8 5.4 2.2l1-1A9 9 0 0 0 8 1 9 9 0 0 0 1.6 3.8l1 1A7.5 7.5 0 0 1 8 2.6Z"/><path d="M8 6c1.2 0 2.3.5 3.1 1.3l1-1A6 6 0 0 0 8 4.6 6 6 0 0 0 3.9 6.3l1 1A4.4 4.4 0 0 1 8 6Z"/><circle cx="8" cy="9.6" r="1.4"/></svg>
        <svg width="25" height="12" viewBox="0 0 25 12" fill="none"><rect x="1" y="1" width="20" height="10" rx="3" stroke="currentColor" strokeOpacity=".4"/><rect x="2.6" y="2.6" width="15" height="6.8" rx="1.6" fill="currentColor"/><path d="M23 4v4c.8-.3 1.4-1 1.4-2S23.8 4.3 23 4Z" fill="currentColor" fillOpacity=".5"/></svg>
      </div>
    </div>
  );
}
...
function Device({ children, flow = true, animOn = true, themeKey }) {
  ...
  return (
    <div className="device">
      ...
      {flow && <canvas ref={cv} className="flow-bg" />}
...
function Stat({ k, v, d, dpos, sm }) {
  return (
    <div className="card card-pad solid">
      <div className="stat-k">{k}</div>
      <div className={'stat-v num' + (sm ? ' sm' : '')}>{v}</div>
      {d && <div className={'stat-d ' + (dpos ? 'pos' : 'neg')}>{d}</div>}
    </div>
  );
}
...
function Row({ icon, iconClass, title, sub, val, valClass, right, onClick }) {
  return (
    <div className="row" onClick={onClick} style={onClick ? { cursor: 'pointer' } : null}>
      {icon && <div className={'row-ic ' + (iconClass || '')}><Icon name={icon} size={20} /></div>}
      <div className="row-tx"><b>{title}</b>{sub && <span>{sub}</span>}</div>
      {val && <div className={'row-val ' + (valClass || '')}>{val}</div>}
      {right}
    </div>
  );
}
...
function Toggle({ on, onChange }) {
  return <button className="tgl" data-on={on ? '1' : '0'} onClick={() => onChange(!on)}><i /></button>;
}

function Bars({ data, run = true }) {
  // data: [{h:0-100, kind:'lo'|'hi'|'sell'}]
  const ref = useRef(null);
  useEffect(() => {
    const id = setTimeout(() => {
      ref.current?.querySelectorAll('.bar').forEach((b, i) => {
        b.style.height = (run ? (data[i]?.h ?? 0) : data[i]?.h ?? 0) + '%';
      });
    }, run ? 120 : 0);
  ...
  return (
    <div className="bars" ref={ref}>
      {data.map((d, i) => <div key={i} className={'bar ' + (d.kind || 'lo')} style={{ height: run ? 0 : d.h + '%' }} />)}
    </div>
  );
}

/* price curve with shaded cheap/expensive windows + now marker */
function PriceCurve({ points, now = 0.5, run = true }) {
  // points: array of 0..1 (price normalized). builds a smooth path.
  ...
  useEffect(() => {
    ...
    p.style.strokeDasharray = len; p.style.strokeDashoffset = run ? len : 0;
    requestAnimationFrame(() => { p.style.transition = 'stroke-dashoffset 1.3s ease'; p.style.strokeDashoffset = 0; });
    // fallback so the line is never left hidden on a throttled clock
    const fb = setTimeout(() => { p.style.strokeDashoffset = 0; }, 1600);
    return () => clearTimeout(fb);
  }, [run]);
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} className="price-track" preserveAspectRatio="none">
      <defs>
      ...
      <line x1={nowX} y1="2" x2={nowX} y2={H - 2} stroke="var(--ink-3)" strokeWidth="1" strokeDasharray="3 3" opacity=".5" />
      <circle className="price-now-dot" cx={nowX} cy={nowY} r="5.5" fill="var(--accent)" stroke="var(--card)" strokeWidth="2.5" />
    </svg>
  );
}
