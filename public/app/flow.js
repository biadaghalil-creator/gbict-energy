...
(function () {
  function initEnergyFlow(canvas) {
    ...
    function resize() {
      ...
      const n = Math.min(900, Math.round(w * h / 1300));
      parts = [];
      ...
      for (let pass = 0; pass < 20; pass++) {
        ...
        t += 90;
        for (let s = 0; s < 7; s++) step(false);
      }
    }
    function spawn(any) {
      return { x: Math.random() * w, y: Math.random() * h, z: Math.random(), life: (any ? Math.random() : 1) * 220 + 40 };
    }
    ...
    function step(fade) {
      if (fade) { ctx.fillStyle = "rgba(" + bg + ",0.05)"; ctx.fillRect(0, 0, w, h); }
      ctx.lineCap = "round";
      for (const p of parts) {
        const a = angle(p.x, p.y);
        const sp = 0.5 + p.z * 1.6;
        const nx = p.x + Math.cos(a) * sp, ny = p.y + Math.sin(a) * sp;
        ctx.strokeStyle = "rgba(" + acc + "," + (0.14 + p.z * 0.42).toFixed(3) + ")";
        ctx.lineWidth = 0.6 + p.z * 2.1;
